...
| ------------------------------------- | ----------------------- | ----------------------------------------------- |
| `general_l1_rpc_url`                  | `http://localhost:8545` | Settlement-layer JSON-RPC endpoint for the node |
...
{% hint style="info" %}
For L3 chains, `general_l1_rpc_url` points to the settlement layer. On ADI, that means ADI Chain. For L2 external nodes, it points to Ethereum. The variable name is inherited from zkSync OS upstream.
{% endhint %}
...
| Variable                           | Default | Description                                                                               |
| ---------------------------------- | ------- | ----------------------------------------------------------------------------------------- |
| `l1_watcher_poll_interval`         | `100ms` | Poll interval for settlement-layer watchers. Good for local dev. Use `2s` for public RPCs |
| `l1_watcher_max_blocks_to_process` | `1000`  | Maximum blocks processed per poll. Good for local dev. Use `10` for public endpoints      |
...
{% hint style="info" %}
`l1_chain_id` in `genesis.json` is the chain ID of your settlement layer. For L3s on ADI, use `36900` on mainnet or `99999` on testnet. `adi init` sets this automatically.
{% endhint %}
...
| `SETTLEMENT_RPC_URL`    | —                | Settlement-layer RPC URL for batch tracking |
...
| `hyperchainsConfig[].tokens`                    | List of bridgeable tokens with addresses and metadata. The UI defaults to the settlement layer's base token unless you add more entries |
